...
#include <stdlib.h>
#include <string.h>
#include <omp.h>  // Para medir tiempo si se desea
...
char* read_file(const char* filename, int* file_size) {
    ...
    *file_size = ftell(file); // Tamaño del archivo
    ...
    char* content = (char*)malloc(*file_size + 2); // +1 para '\1' y +1 para '\0'
    ...
    *file_size += 1; // Ahora el tamaño incluye el símbolo de fin
    return content;
}
...
int* build_suffix_array(const char* text, int n) {
    ...
    // Ordenar sufijos usando comparación personalizada
    qsort_r(suffix_array, n, sizeof(int), suffix_compare, (void*)text);
    return suffix_array;
}

// Aplica la transformación de Burrows-Wheeler usando suffix array
...
int main(int argc, char* argv[]) {
    ...
    printf("Transformación BWT completada.\n");
